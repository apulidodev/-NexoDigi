...
export default async function Home() {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f7f1e7] text-[#172539]">
    <nav className="mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
      ...
      <div className="hidden items-center gap-7 font-mono text-xs font-bold uppercase tracking-widest md:flex"><a href="#archivo">Archivo</a><a href="#tamer">Tamer</a><a href="#nexorift">Run</a><a href="#mision">Misión</a><a href="#comunidad">Comunidad</a><a href="#medallas">Medallas</a></div>
      <div className="hidden items-center gap-2 md:flex"><CollectionShortcut /><NotificationBell /><TamerAccess /></div>
      <MobileNav />
    </nav>
    ...
    <MotionReveal><section id="mision" className="mx-auto grid max-w-7xl gap-8 px-4 py-14 sm:px-5 sm:py-20 lg:grid-cols-[1.1fr_0.9fr] lg:px-8"><div className="min-w-0 rounded-[2rem] border-2 border-[#172539] bg-[#f36d57] p-6 shadow-[6px_6px_0_#172539] sm:p-10 sm:shadow-[8px_8px_0_#172539]"><p className="font-mono text-xs font-black uppercase tracking-[0.16em]">Misión de bienvenida</p><div className="mt-6 flex items-start gap-3"><h2 className="max-w-lg font-mono text-3xl font-black uppercase leading-none tracking-[-0.07em] sm:text-5xl">Encuentra el Digimon que defina tu expedición.</h2><InfoHint className="mt-1" text="Abrir el radar te lleva al DigiDex y activa el campo de búsqueda." /></div><div className="mt-8"><RadarButton /></div></div><div id="comunidad" className="min-w-0 rounded-[2rem] border-2 border-[#172539] bg-white p-5 shadow-[4px_4px_0_#172539] sm:p-10 sm:shadow-none"><Badge>COMUNIDAD</Badge><div className="mt-5 flex min-w-0 items-start gap-2 sm:gap-3"><h2 className="min-w-0 break-words font-mono text-2xl font-black uppercase leading-none tracking-[-0.06em] sm:text-4xl">El archivo crece con cada Tamer.</h2><InfoHint className="mt-1" text="Consulta apariciones verificadas por la comunidad o, al iniciar sesión, propone una nueva para revisión del staff." /></div><CommunityHub /></div></section></MotionReveal>
