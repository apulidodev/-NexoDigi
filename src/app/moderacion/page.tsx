...
export default function ModerationPage() {
  ...
  const decide = async (suggestionId: string, decision: "approved" | "rejected" | "changes_requested") => {
    ...
    setBusy(null);
    if (!response.ok) { setMessage(data.error ?? "No fue posible guardar la decisiÃ³n."); return; }
    setMessage(decision === "approved" ? "ApariciÃ³n aprobada y publicada." : "DecisiÃ³n guardada.");
    await load();
  };

  return <main className="min-h-screen bg-[#f7f1e7] px-5 py-12 text-[#172539]"><section className="mx-auto max-w-4xl rounded-[2rem] border-2 border-[#172539] bg-white p-6 shadow-[8px_8px_0_#172539] sm:p-10"><Link href="/#comunidad" className="font-mono text-xs font-black uppercase underline">â† Volver a comunidad</Link><p className="mt-8 font-mono text-xs font-black uppercase tracking-[0.16em] text-[#d85746]">Staff NexoDigi</p><h1 className="mt-3 font-mono text-3xl font-black uppercase tracking-[-0.06em] sm:text-5xl">Cola de moderaciÃ³n</h1><p className="mt-4 max-w-2xl leading-7 text-[#405065]">Aprueba solo apariciones con fuente verificable. Cada decisiÃ³n queda registrada en la bitÃ¡cora de auditorÃ­a.</p><Link href="/moderacion/control" className="mr-4 font-mono text-xs font-black uppercase underline">Control de temporadas â†’</Link><Link href="/moderacion/medallas" className="mr-4 font-mono text-xs font-black uppercase underline">Forja de insignias →</Link><Link href="/moderacion/reportes" className="mt-4 inline-block font-mono text-xs font-black uppercase underline">Ver reportes comunitarios â†’</Link>{message && <p className="mt-6 rounded-xl border-2 border-[#172539] bg-[#dcebdc] p-3 font-mono text-xs font-bold" role="status">{message}</p>}<div className="mt-7 grid gap-4">{suggestions.map((item) => <article key={item.id} className="rounded-2xl border-2 border-[#172539] bg-[#f7f1e7] p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-xs font-black uppercase">DAPI #{item.dapi_id} Â· {item.series?.name ?? "Serie desconocida"}</p><p className="mt-2 text-sm">{item.character_name && `${item.character_name} Â· `}{item.kind}{item.season ? ` Â· Temporada ${item.season}` : ""}</p></div><span className="font-mono text-[10px] uppercase text-[#405065]">{new Date(item.created_at).toLocaleDateString("es-MX")}</span></div><p className="mt-4 text-sm leading-6">{item.explanation}</p><ul className="mt-3 space-y-1 text-sm">{item.sources.map((source) => <li key={source.id}><a className="underline" href={source.url} target="_blank" rel="noreferrer">Fuente: {source.title}</a></li>)}</ul><div className="mt-5 flex flex-wrap gap-2"><Button disabled={busy === item.id} onClick={() => void decide(item.id, "approved")}>Aprobar</Button><Button variant="outline" disabled={busy === item.id} onClick={() => void decide(item.id, "changes_requested")}>Pedir cambios</Button><Button variant="outline" disabled={busy === item.id} onClick={() => void decide(item.id, "rejected")}>Rechazar</Button></div></article>)}</div></section></main>;
}
