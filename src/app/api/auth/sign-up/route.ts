...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase.auth.signUp({ email: input.data.email, password: input.data.password, options: { data: { handle: input.data.handle }, emailRedirectTo: new URL("/auth/callback", origin).toString() } });
