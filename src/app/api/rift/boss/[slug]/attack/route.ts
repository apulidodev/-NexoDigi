...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!user) return NextResponse.json({ error: "Inicia sesiÃ³n para atacar al jefe global." }, { status: 401 });
  ...
  if (!event || event.current_hp <= 0) return NextResponse.json({ error: "El evento ya no estÃ¡ activo." }, { status: 409 });
  ...
  if (updateError) return NextResponse.json({ error: "El jefe recibiÃ³ otra seÃ±al. Intenta de nuevo." }, { status: 409 });
