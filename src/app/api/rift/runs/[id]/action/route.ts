...
import { z } from "zod";
import { resolveRiftAction, type RiftAction, type RiftState } from "@/features/digital-run/application/rift-server-combat";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const outcome = { ...result, node: next.node, enemyMaxHp: next.status === "active" ? undefined : 0 };
