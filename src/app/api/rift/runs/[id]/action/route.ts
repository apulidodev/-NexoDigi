...
import { z } from "zod";
import { dailyEnemyId, enemyMaxHp, resolveRiftAction, type RiftAction, type RiftState } from "@/features/digital-run/application/rift-server-combat";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!input.success) return NextResponse.json({ error: "AcciÃ³n invÃ¡lida." }, { status: 400 });
  ...
  if (run.status !== "active") return NextResponse.json({ error: "La ruta ya terminÃ³." }, { status: 409 });
  ...
  if (writeError) return NextResponse.json({ error: "No fue posible registrar la acciÃ³n." }, { status: 500 });
  ...
  if (actionError) return NextResponse.json({ error: "La acciÃ³n fue procesada; vuelve a consultar la ruta." }, { status: 409 });
  ...
  if (next.status === "completed") await awardBadges(admin, { userId: user.id, key: "rift_completed", sourceId: run.id });
