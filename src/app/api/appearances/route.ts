...
export async function GET(request: Request) {
  ...
  if (!Number.isInteger(dapiId) || dapiId <= 0) return NextResponse.json({ error: "dapiId invÃ¡lido." }, { status: 400 });
...
export async function POST(request: Request) {
  ...
  if (!input.success) return NextResponse.json({ error: "Sugerencia invÃ¡lida.", details: input.error.flatten() }, { status: 400 });
  const supabase = await createSupabaseServerClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "Inicia sesiÃ³n para proponer una apariciÃ³n." }, { status: 401 });
  ...
  if (sourcesError) return NextResponse.json({ error: "La sugerencia se guardÃ³, pero fallaron sus fuentes." }, { status: 500 });
  await supabase.from("notifications").insert({ user_id: user.id, kind: "system", title: "Propuesta recibida", body: "Tu apariciÃ³n fue enviada a revisiÃ³n del staff.", href: "/#comunidad" });
  await awardBadges(createSupabaseAdminClient(), { userId: user.id, key: "appearance_proposed", sourceId: data.id });
