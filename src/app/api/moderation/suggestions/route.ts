...
export async function GET(request: Request) {
  ...
  if (!["pending", "changes_requested", "approved", "rejected"].includes(status)) return NextResponse.json({ error: "Estado invÃ¡lido." }, { status: 400 });
...
export async function PATCH(request: Request) {
  ...
  if (!input.success) return NextResponse.json({ error: "DecisiÃ³n invÃ¡lida.", details: input.error.flatten() }, { status: 400 });
  const admin = createSupabaseAdminClient();
  ...
  if (input.data.decision === "approved") {
    ...
    if (appearanceError || !appearance) return NextResponse.json({ error: "No fue posible publicar la apariciÃ³n. Revisa si ya existe." }, { status: 409 });
    ...
    if (sources?.length) {
      ...
      if (sourcesError) return NextResponse.json({ error: "La apariciÃ³n fue creada, pero sus fuentes no pudieron copiarse." }, { status: 500 });
    }
  }
  ...
  if (updateError) return NextResponse.json({ error: "No fue posible registrar la decisiÃ³n." }, { status: 500 });
  ...
  await admin.from("notifications").insert({ user_id: suggestion.submitted_by, kind: "moderation", title: input.data.decision === "approved" ? "ApariciÃ³n publicada" : "ActualizaciÃ³n de tu propuesta", body: input.data.decision === "approved" ? "Tu propuesta fue verificada y ya aparece en el archivo comunitario." : input.data.decision === "changes_requested" ? "El staff solicitÃ³ cambios en tu propuesta." : "Tu propuesta no fue aprobada en esta revisiÃ³n.", href: "/#comunidad" });
