...
export function ArchiveExplorer({ catalogs }: ArchiveExplorerProps) {
  ...
  return <section id="archivo-categorias" className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="grid gap-7 lg:grid-cols-[0.75fr_1.25fr]"><div><Badge>ARCHIVO DE DATOS</Badge><h2 className="mt-5 font-mono text-3xl font-black uppercase leading-none tracking-[-0.07em] sm:text-5xl">Explora el archivo de datos.</h2><p className="mt-5 max-w-md leading-7 text-[#405065]">Elige una categoria para consultar sus registros oficiales. Los catalogos grandes se cargan por partes para que la exploracion sea rapida.</p><div className="mt-7 flex flex-wrap gap-2">{catalogsState.map((catalog) => <button key={catalog.kind} type="button" onClick={() => selectKind(catalog.kind)} className={`rounded-full border-2 border-[#172539] px-3 py-2 font-mono text-[10px] font-black uppercase tracking-wider transition-colors ${activeKind === catalog.kind ? "bg-[#fdcc48]" : "bg-white hover:bg-[#79c8ea]"}`}>{catalog.name} <span className="opacity-60">{catalog.totalElements.toLocaleString("es-MX")}</span></button>)}</div></div><div className="rounded-3xl border-2 border-[#172539] bg-white p-5 shadow-[6px_6px_0_#172539]"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-xs font-black uppercase tracking-widest text-[#d85746]">{activeCatalog.name}</p><p className="mt-2 max-w-xl text-sm leading-6 text-[#405065]">{activeCatalog.description}</p></div><Badge className="shrink-0 bg-[#dcebdc]">{activeCatalog.isAvailable ? `${activeCatalog.entries.length.toLocaleString("es-MX")} de ${activeCatalog.totalElements.toLocaleString("es-MX")} cargados` : "Sin conexion"}</Badge></div><input value={query} onChange={(event) => setQuery(event.target.value)} className="mt-5 h-11 w-full rounded-xl border-2 border-[#172539] bg-[#f7f1e7] px-4 font-mono text-sm outline-none focus:ring-2 focus:ring-[#79c8ea]" placeholder={`Buscar entre los ${activeCatalog.entries.length.toLocaleString("es-MX")} registros cargados...`} /><div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3">{entries.map((entry) => <article key={entry.id} className="rounded-xl border-2 border-[#172539] bg-[#f7f1e7] px-3 py-2 font-mono text-xs font-black uppercase shadow-[2px_2px_0_#172539]"><span className="mr-2 text-[#d85746]">{String(entry.id).padStart(3, "0")}</span>{entry.name}</article>)}</div>{entries.length === 0 && <p className="mt-6 rounded-xl border-2 border-dashed border-[#172539] p-6 text-center font-mono text-sm">{activeCatalog.isAvailable ? "No encontramos coincidencias entre los registros cargados. Prueba otra busqueda o carga mas." : "No pudimos cargar este catalogo desde DAPI. Intenta de nuevo mas tarde."}</p>}{activeCatalog.hasNextPage && <div className="mt-6 text-center"><Button variant="outline" disabled={isLoadingMore} onClick={() => void loadMore()}>{isLoadingMore ? "Cargando..." : "Cargar mas registros"}</Button></div>}</div></div></section>;
}