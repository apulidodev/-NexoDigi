...
import { AttackSequence, EvolutionOverlay, ModuleOrbit, TimingMeter, useRunSounds } from "@/components/digital-run/run-effects";
import { Button } from "@/components/ui/button";
import { InfoHint } from "@/components/ui/info-hint";
import { RiftDailyStatus } from "@/components/digital-run/rift-daily-status";
import { OnlineRiftPanel } from "@/components/digital-run/online-rift-panel";
import { GlobalBossPanel } from "@/components/digital-run/global-boss-panel";
import { RiftRankings } from "@/components/digital-run/rift-rankings";
import { PixiRiftParticles } from "@/components/digital-run/pixi-rift-particles";
import { RiftCombatFlow, RiftOutcomeCard, RiftRewardDeck } from "@/components/digital-run/rift-combat-flow";
...
type EvolutionScene = { targetName: string; sourceImage?: string; targetImage?: string };
...
export function DigitalRunGame() {
  ...
  useEffect(() => {
    void fetch("/api/rift/runs").then((response) => response.ok ? response.json() as Promise<{ runs: OnlineRun[] }> : { runs: [] }).then(async ({ runs }) => {
  ...
  async function startOnline() {
    if (!selectedPartnerId) { setError("Elige un compañero antes de entrar al Rift online."); return; }
    setError(""); setIsOnlineBusy(true); setOnlineMessage("");
    try {
  ...
  async function resolveOnlineAction(action: "pulse" | "technique" | "guard") {
    if (!onlineRun || onlineRun.status !== "active" || isOnlineBusy) return;
