...
export function TamerAccess() {
  ...
  if (session) return <div className="flex items-center gap-2"><a href="/perfil" className="hidden rounded-lg border-2 border-[#172539] bg-white px-2 py-1 font-mono text-[10px] font-black uppercase sm:inline">Perfil</a><span className="hidden max-w-28 truncate font-mono text-[10px] font-black uppercase sm:inline">@{session.profile?.handle ?? "tamer"}</span>{["moderator", "admin"].includes(session.profile?.role ?? "") && <a href="/moderacion" className="hidden rounded-lg border-2 border-[#172539] bg-[#fdcc48] px-2 py-1 font-mono text-[10px] font-black uppercase sm:inline">Staff</a>}<Button size="sm" variant="outline" onClick={() => void signOut()}>Salir</Button></div>;
